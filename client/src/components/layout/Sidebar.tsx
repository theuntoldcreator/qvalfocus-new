...
export function Sidebar({ isOpen, onClose, navLinks }: SidebarProps) {
  ...
  const sidebarContent = (
    <>
      {/* Header with Logo and Close Button (only for mobile overlay) */}
      {isMobile && (
      ...
      {/* Logo and Tagline (for desktop fixed sidebar) */}
      {!isMobile && (
  ...
  // Render for desktop (fixed sidebar)
  if (!isMobile) {
    return (
      <aside className="hidden md:flex md:flex-col md:w-64 md:flex-shrink-0 md:bg-white dark:md:bg-slate-900 md:shadow-lg md:border-r md:border-slate-200 dark:md:border-slate-800 md:h-screen md:sticky md:top-0 md:overflow-y-auto p-6">
        {sidebarContent}
      </aside>
    );
  }

  // Render for mobile (overlay)
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
        ...
        >
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-3/4 max-w-sm bg-white dark:bg-slate-900 shadow-lg p-6 overflow-y-auto flex flex-col"
